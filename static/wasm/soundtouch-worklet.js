...
var SoundTouchWorklet = function (_AudioWorkletProcesso) {
  function SoundTouchWorklet() {
    ...
    _this._lastFrameL = 0;
    _this._lastFrameR = 0;
    _this._maxProcessPasses = 8;
    _this._lastTempo = 1.0;
    _this._lastPitch = 1.0;
    _this._lastRate = 1.0;
    _this._lastSemitone = 0;
    return _this;
  }
  ...
  return _createClass(SoundTouchWorklet, [{
    ...
    value: function process(inputs, outputs, parameters) {
      ...
      this._pipe.process();
      var framesNeeded = leftOutput.length;
      var framesFilled = 0;
      var processPasses = 0;
      while (framesFilled < framesNeeded && processPasses < this._maxProcessPasses) {
        ...
        var framesToWrite = Math.min(framesNeeded - framesFilled, availableFrames, this.bufferSize);
        this._pipe.outputBuffer.receiveSamples(this._processedSamples, framesToWrite);
        for (var frame = 0; frame < framesToWrite; frame++) {
          var outIndex = framesFilled + frame;
          var sampleIndex = frame * 2;
          var leftSample = this._processedSamples[sampleIndex];
          var rightSample = this._processedSamples[sampleIndex + 1];
          if (!isFinite(leftSample)) {
            leftSample = this._lastFrameL;
          }
          if (!isFinite(rightSample)) {
            rightSample = this._lastFrameR;
          }
          leftOutput[outIndex] = leftSample;
          rightOutput[outIndex] = rightSample;
          this._lastFrameL = leftSample;
          this._lastFrameR = rightSample;
        }
        framesFilled += framesToWrite;
      }
      if (framesFilled < framesNeeded) {
        for (var i = framesFilled; i < framesNeeded; i++) {
          leftOutput[i] = this._lastFrameL;
          rightOutput[i] = this._lastFrameR;
        }
      }
      return true;
    }
    ...
    value: function _handleParameterChange(_ref) {
      var rate = _ref.rate,
        tempo = _ref.tempo,
        pitch = _ref.pitch,
        pitchSemitones = _ref.pitchSemitones;
      var threshold = 1e-3;
      var tempoChanged = Math.abs(tempo - this._lastTempo) > threshold;
      var pitchChanged = Math.abs(pitch - this._lastPitch) > threshold || Math.abs(pitchSemitones - this._lastSemitone) > 0.01;
      var rateChanged = Math.abs(rate - this._lastRate) > threshold;
      if (tempoChanged || pitchChanged || rateChanged) {
        this._flushPipeBuffers();
        this._lastTempo = tempo;
        this._lastPitch = pitch;
        this._lastRate = rate;
        this._lastSemitone = pitchSemitones;
      }
    }
  }, {
    key: "_flushPipeBuffers",
    value: function _flushPipeBuffers() {
      var _this$_pipe$outputBuf, _this$_pipe$_intermed;
      this._pipe.transposer.clear();
      this._pipe.stretch.clear();
      var inputBuffer = this._pipe.inputBuffer;
      if (inputBuffer) {
        var framesToKeep = Math.min(inputBuffer.frameCount, this.bufferSize * 8);
        var framesToDrop = inputBuffer.frameCount - framesToKeep;
        if (framesToDrop > 0) {
          inputBuffer.receive(framesToDrop);
        }
      }
      (_this$_pipe$outputBuf = this._pipe.outputBuffer) === null || _this$_pipe$outputBuf === void 0 || _this$_pipe$outputBuf.clear();
      (_this$_pipe$_intermed = this._pipe._intermediateBuffer) === null || _this$_pipe$_intermed === void 0 || _this$_pipe$_intermed.clear();
    }
  }], [{
