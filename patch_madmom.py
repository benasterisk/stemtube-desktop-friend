...
def main():
    ...
    # Search both Linux (lib/python3.X) and Windows (Lib) layouts
    import platform
    candidates = []
    if platform.system() == 'Windows':
        candidates.append(os.path.join(venv_path, 'Lib', 'site-packages', 'madmom'))
    python_lib_dirs = glob.glob(os.path.join(venv_path, 'lib', 'python3.*'))
    for lib_dir in python_lib_dirs:
        candidates.append(os.path.join(lib_dir, 'site-packages', 'madmom'))
